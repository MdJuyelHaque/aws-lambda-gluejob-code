...
HISTORICAL_FOLDER = os.environ.get('HISTORICAL_FOLDER')
...
# BUCKET = 'bti-dl'
# HISTORICAL_FOLDER = 'Historical_Data_From_Teams/'


# # #RDS settings Test 
# HOST = 'bti-staging-1.c0vqzjmbxrqq.us-west-2.rds.amazonaws.com' #'localhost'
# USER_NAME = 'root'
# PASSWORD = ')#RWvTYd3k'
# # DB_NAME = 'bti_powerBI_test' #'historicaldata'
# DB_NAME = 'bti_others_staging' #'historicaldata'

# TABLE_NAME = 'bti_historical_3clogic'
...
def get_files_list_from_historical():
    files_list = []
    try:
        print("Fetching files list from Historical_Data_From_Teams started...")
        logger.info("Fetching files list from Historical_Data_From_Teams started...")
        objs = list(s3_bucket.objects.filter(Prefix=HISTORICAL_FOLDER))
        for obj in objs:
            files_list.append(obj.key)
        print(files_list)
        logger.info("SUCCESS: Fetching files list from Historical_Data_From_Teams completed.")
        print("SUCCESS: Fetching files list from Historical_Data_From_Teams completed.")
        return files_list
    except Exception as e:
        logger.error("ERROR: Unexpected error: Could not get files list from Historical_Data_From_Teams.")
        logger.error(e)
        sys.exit()
...
def load_to_aurora(s3_file_path):
    try:
        if s3_file_path.split('/')[-1].split('.')[-1] == "csv":
            ...
            # date_value = datetime.strptime(str(previous_day),'%Y-%M-%d').strftime('%M/%d/%Y')
            # df['CurrDate']=date_value
...
def historical_to_aurora():
    files_list = get_files_list_from_historical()
...
def handler(event, context):
   # pload=logpayload(event)
    #publish_to_aurora()
    historical_to_aurora()
    logger.info("----Ended----")
    print("-----3CLogic_to_Aurora_Staging_Ended-----")
    #user = gluejob(event)
    print("Successfull")
    return "Successful"
...
#     historical_to_aurora()
#     print("-----3CLogic_Names_to_Aurora_Staging_Ended-----")
