...
def authenticate():
    obj = s3Resource.Object('conf-token', "access-token-infusionSoft_HO")
...
def data_extraction(accessToken, refreshToken, isError):
    try:
        jobHistory = get_data_dynamoDB('bti_job_history_InfusionSoft_IS1')
        # If function is called for the first time or after refreshing a token
        if not isError:
            apiDetails = get_data_dynamoDB('bti_job_config_InfusionSoft_IS1')
        else:
...
def refresh_token(refreshToken):
    try:
        ...
        s3Client.put_object(Body=encryptedData, Bucket='conf-token', Key='access-token-infusionSoft_HO')
...
def log_generator(log):
    ...
    table = dynamodb.Table('bti_job_history_InfusionSoft_IS1')
...
# handler
def extract_load_infusionSoft_ho(events, context):
    authenticate()
    logger.info("----Ended----")
