...
# Returns primary key column for tables
def get_primary_key_table_column(table_name):
    switcher = {
        "affiliateCommissions": "invoice_id",
        "countries": "country_code",
        "merchants": "merchant_accounts_id",
        "contacts" : "custom_fields_id",
        "userInfo": "global_user_id"
    }
    return switcher.get(table_name, "id")
...
def create_table(table_name, df):
    ...
    try:
        ...
        # create_table_query += ");"
        create_table_query += ", PRIMARY KEY (" + get_primary_key_table_column(table_name) + "));"
...
def load_to_aurora(s3_file_path):
    try:
        ...
        if s3_file_path.split('/')[-1].split('.')[-1] == "csv" and today_str in s3_file_path:
            table_name = extract_table_name(s3_file_path)
