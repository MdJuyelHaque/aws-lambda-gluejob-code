    ...
    try:
        ...
        for each_element in apiDetails:
            ...
            if api_status or api_failed_status:
                ...
                ## To convert <class 'xmlrpc.client.DateTime'> to python timestamp str
                for each_element1 in data_to_upload:
                    temp=each_element1['PayDate']
                    str_t= str(temp)
                    time_py= str_t[0:4]+'-'+str_t[4:6]+'-'+str_t[6:8]+'T00:00:00.00Z'
                    each_element1['PayDate']=time_py
                
                    temp=each_element1['LastUpdated']
                    str_t= str(temp)
                    time_py= str_t[0:4]+'-'+str_t[4:6]+'-'+str_t[6:8]+'T00:00:00.00Z'
                    each_element1['LastUpdated']=time_py
                print('-------------Table name to be uploaded------'+each_element['api'])
                print('##################### Checkpoint 2 ########################')
                upload(each_element['api'], json.dumps(data_to_upload), current_timestamp, "json")
...
# handler
def extract_load_infusionSoft(event, context):
    # pload = logpayload(event)
    authenticate()
    #logger.info("----Ended----")
    logger.info("----Ended InfusionSoft_jj_Set3_EL----")
    print("----Ended InfusionSoft_jj_Set3_EL----")
    print("----Ended----")
    print("Successfull")
