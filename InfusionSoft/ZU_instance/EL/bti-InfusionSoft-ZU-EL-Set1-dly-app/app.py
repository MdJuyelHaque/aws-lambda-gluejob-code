...
def authenticate():
    obj = s3Resource.Object('conf-token', "access-token-infusionSoft_ZU")
...
def data_extraction(accessToken, refreshToken, isError):
    try:
        jobHistory = get_data_dynamoDB('bti_job_history_InfusionSoft_IS3')
        # If function is called for the first time or after refreshing a token
        if not isError:
            apiDetails = get_data_dynamoDB('bti_job_config_InfusionSoft_IS3')
        else:
        ...
        for each_element in apiDetails:
            ...
            if api_status or api_failed_status:
                ...
                url = host + each_element['endpoint'] + 'access_token=' + accessToken + '&since=' + time_obj[
                    'since'] + '&until=' + time_obj['until']
                    ...
                    if isinstance(response[key], list):
                        ...
                        if 'next' in response.keys():
                            if response['next'] != None:
                                ...
                                while next_url != None:
                                    urlNext = next_url + '&access_token=' + accessToken
                                    res = requests.get(urlNext)
                                    if res:
                                        res = res.json()
                                        res_key = list(res.keys())[0]
                                        if len(res[res_key]) == 0 or res['next'] == None:
                                            next_url = None
                                            data += res[key]
                                            rows_extracted = len(data)
                                        else:
...
def refresh_token(refreshToken):
    try:
        ...
        s3Client.put_object(Body=encryptedData, Bucket='conf-token', Key='access-token-infusionSoft_ZU')
...
def log_generator(log):
    ...
    table = dynamodb.Table('bti_job_history_InfusionSoft_IS3')
...
# handler
def extract_load_infusionSoft_zu(events, context):
    authenticate()
    logger.info("----Ended----")
